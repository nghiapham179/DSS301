...
def get_flight_status(battery_level, signal_strength, wind_speed, gps_accuracy, flight_time, temperature, altitude, speed, risk_pred, maint_pred):
    maint_text = str(maint_pred).lower()
    if battery_level < 20 or signal_strength < 35 or wind_speed > 35 or gps_accuracy > 10 or temperature > 45 or temperature < -10:
        return "Cấm Bay", "Drone không nên bay do pin thấp, tín hiệu yếu, gió mạnh, GPS kém hoặc nhiệt độ nguy hiểm."
    if "urgent" in maint_text or "required" in maint_text or "maintenance" in maint_text or "inspect" in maint_text or "inspection" in maint_text:
    ...
    if battery_level < 40 or signal_strength < 60 or wind_speed > 25 or gps_accuracy > 7 or flight_time > 40 or altitude > 350 or speed > 75:
        return "Bay Kèm Giám Sát", "Drone có thể bay nhưng cần theo dõi sát vì một số thông số đang ở mức cần chú ý."
    return "Đủ Điều Kiện Bay", "Drone đủ điều kiện vận hành bình thường."
...
page = st.sidebar.radio(
    "Chọn trang",
    ["Dashboard", "Dự đoán", "Nhập dữ liệu drone", "Phân tích drone"]
)
...
if page == "Dashboard":
    ...
    with col_score:
        render_score_box(
            f"{avg_score:.1f}/10", score_status,
            f"{high_pct}% bản ghi thuộc nhóm High Risk", score_color
        )
    ...
    with col_gauge:
        fig_score = go.Figure(go.Indicator(
            ...
            gauge={
                ...
                "threshold": {"line": {"color": "#111111", "width": 3}, "thickness": 0.75, "value": avg_score}
            }
        ))
        fig_score.update_layout(height=260, margin=dict(t=40, b=10, l=20, r=20), paper_bgcolor="rgba(0,0,0,0)")
        st.plotly_chart(fig_score, width='stretch')
    close_card()

    c1, c2, c3, c4 = st.columns(4)
    with c1: render_metric_card("Tổng bản ghi", f"{len(df):,}", "records")
    with c2: render_metric_card("Số drone", df["drone_id"].nunique(), "drone units")
    with c3: render_metric_card("Risk score TB", f"{avg_score:.1f}", "out of 10")
    with c4: render_metric_card("Tỷ lệ High Risk", f"{high_pct}%", "cần xử lý")
    ...
    with col1:
        open_card()
        render_section_title("Phân phối Risk Score", "Biểu đồ thể hiện mức rủi ro vận hành của drone.")
        fig = px.histogram(df, x="risk_score", color="operation_risk", nbins=11, color_discrete_map={"High": "#E24B4A", "Medium": "#EF9F27", "Low": "#639922"}, barmode="overlay")
    ...
    with col2:
        open_card()
        render_section_title("Maintenance Action", "Phân bố các hành động bảo trì được đề xuất.")
        mc = df["maintenance_action"].value_counts().reset_index()
        mc.columns = ["action", "count"]
        fig2 = px.bar(mc, x="count", y="action", orientation="h", color="action")
    ...
    with col3:
        open_card()
        render_section_title("Battery Level vs Wind Speed", "So sánh pin và tốc độ gió theo mức rủi ro.")
        sample_df = df.sample(min(3000, len(df)), random_state=42)
        fig3 = px.scatter(sample_df, x="battery_level", y="wind_speed", color="operation_risk", opacity=0.45, color_discrete_map={"High": "#E24B4A", "Medium": "#EF9F27", "Low": "#639922"})
        fig3.update_layout(margin=dict(t=10, b=10, l=10, r=10), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)")
        st.plotly_chart(fig3, width='stretch')
        close_card()

    with col4:
        open_card()
        render_section_title("Risk Score trung bình theo Drone", "Mức rủi ro trung bình của từng drone.")
        drone_avg = df.groupby("drone_id")["risk_score"].mean().reset_index()
        fig4 = px.bar(drone_avg, x="drone_id", y="risk_score", color="risk_score", color_continuous_scale="RdYlGn_r")
        fig4.update_layout(margin=dict(t=10, b=10, l=10, r=10), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)")
        st.plotly_chart(fig4, width='stretch')
        close_card()

# ================== PAGE 2: DỰ ĐOÁN ==================
elif page == "Dự đoán":
    render_main_title("Nhập thông số Drone để nhận quyết định", "Hệ thống dự đoán Operation Risk, Maintenance Action, Recommendation và Trạng thái bay.")
    open_card()
    col1, col2 = st.columns(2)
    with col1:
    ...
    flight_status, flight_reason = get_flight_status(
        battery_level, signal_strength, wind_speed, gps_accuracy, flight_time, temperature, altitude, speed, risk_pred, maint_pred
    )
    ...
    bat_icon = {"Tốt": "🟢", "Trung Bình": "🟡", "Yếu": "🔴"}

    est_score = min(10, round((1 - battery_level / 100) * 4 + (wind_speed / 50) * 3 + (1 - signal_strength / 100) * 2 + (abs(temperature) / 50) * 1, 1))

    open_card()
    fig_gauge = go.Figure(go.Indicator(
        mode="gauge+number", value=est_score, title={"text": "Risk Score ước tính"},
        gauge={
            "axis": {"range": [0, 10]}, "bar": {"color": color_map.get(risk_pred, "#378ADD")},
            "steps": [{"range": [0, 2], "color": "#EAF3DE"}, {"range": [2, 6], "color": "#FAEEDA"}, {"range": [6, 10], "color": "#FCEBEB"}],
            "threshold": {"line": {"color": "red", "width": 3}, "thickness": 0.75, "value": 6}
        }
    ))
    fig_gauge.update_layout(height=280, margin=dict(t=40, b=10), paper_bgcolor="rgba(0,0,0,0)")
    st.plotly_chart(fig_gauge, width='stretch')

    r1, r2, r3, r4 = st.columns(4)
    r1.metric(f"{icon_map.get(risk_pred, '⚪')} Mức Rủi Ro", risk_pred_vn, f"Độ tin cậy: {risk_conf}%")
    r2.metric("Hành Động Bảo Trì", maint_pred_vn)
    r3.metric(f"{bat_icon.get(bat_status)} Tình Trạng Pin", bat_status)
    r4.metric("Trạng Thái Bay", flight_status)

    show_flight_status(flight_status, flight_reason)
    st.subheader("Khuyến nghị cuối cùng")
    if risk_pred == "High": st.error(rec_pred)
    elif risk_pred == "Medium": st.warning(rec_pred)
    else: st.success(rec_pred)
    close_card()

# ================== PAGE 3: NHẬP DỮ LIỆU DRONE ==================
elif page == "Nhập dữ liệu drone":
    render_main_title("Nhập dữ liệu Drone thủ công", "Người vận hành có thể nhập trực tiếp thông số drone để hệ thống đưa ra dự đoán.")
    ...
    if submitted:
        ...
        with st.expander("Xem thông tin dữ liệu đã nhập"):
            input_preview = input_data.copy()
            input_preview.insert(0, "drone_id", drone_id_input)
            st.dataframe(input_preview, width='stretch')

# ================== PAGE 4: PHÂN TÍCH DRONE ==================
elif page == "Phân tích drone":
    render_main_title("Phân tích theo từng Drone", "Theo dõi mức rủi ro, pin và hành động bảo trì của từng drone.")
    open_card()
    drone_sel = st.selectbox("Chọn drone", sorted(df["drone_id"].unique()))
    close_card()

    df_d = df[df["drone_id"] == drone_sel]

    c1, c2, c3, c4 = st.columns(4)
    with c1: render_metric_card("Số bản ghi", f"{len(df_d):,}", drone_sel)
    with c2: render_metric_card("Risk score TB", f"{df_d['risk_score'].mean():.1f}", "average risk")
    with c3: render_metric_card("High Risk", f"{round(df_d['is_high_risk'].mean() * 100, 1)}%", "risk ratio")
    with c4: render_metric_card("Battery TB", f"{df_d['battery_level'].mean():.0f}%", "average battery")
    ...
    with col1:
        open_card()
        render_section_title(f"Risk Score - {drone_sel}", "Phân phối điểm rủi ro của drone được chọn.")
        fig = px.histogram(df_d, x="risk_score", nbins=11, color_discrete_sequence=["#378ADD"])
        fig.update_layout(margin=dict(t=10, b=10, l=10, r=10), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)")
        st.plotly_chart(fig, width='stretch')
        close_card()
    ...
    with col2:
        open_card()
        render_section_title(f"Flight Time vs Battery - {drone_sel}", "Mối quan hệ giữa thời gian bay và pin.")
        sample_drone = df_d.sample(min(1000, len(df_d)), random_state=42)
        fig2 = px.scatter(sample_drone, x="flight_time", y="battery_level", color="operation_risk", color_discrete_map={"High": "#E24B4A", "Medium": "#EF9F27", "Low": "#639922"})
        fig2.update_layout(margin=dict(t=10, b=10, l=10, r=10), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)")
        st.plotly_chart(fig2, width='stretch')
        close_card()

    open_card()
    render_section_title("Phân bố Maintenance Action", "Tỷ lệ các hành động bảo trì được đề xuất.")
